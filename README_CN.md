...
从零实现的轻量级 **Agent Harness** —— 不依赖 LangChain、AutoGen、CrewAI。
...
│   │   ├── recovery.py              # RecoveryManager 错误恢复
...
│
│   # src/miniclaw/*.py 中保留部分旧版兼容模块，供早期 demo/测试使用。
│   # 新代码优先使用 agent/、tools/、storage/、memory/ 这些新版包结构。
│
└── tests/                           # 25 个测试文件，466 个测试用例
```
