...
MiniClaw 将以上每个问题实现为独立、可测试的模块。整个系统运行在单机上，除 LLM 端点外不依赖任何外部服务，通过 660+ 项单元测试。
...
- `miniclaw trace html` 可导出自包含 HTML 报告，并包含 Mermaid trace 时间线。
...
- `ToolExecutor` 支持工具级 wall-clock timeout 和协作取消。
- `SandboxExecutor` 提供受限 Shell 执行模式，包含命令黑名单、环境变量净化和路径边界。
- `VectorMemoryBackend` 提供无外部依赖的可选向量检索记忆，适合 demo 和测试。
- `CompositeMemoryBackend` 可组合两个记忆后端，并支持 replace/merge 冲突处理。
- `LLMMemoryExtractor` 可使用 LLM 做语义记忆抽取，同时保留敏感信息过滤。
- `OpenAPIToolRegistry` 可从 JSON/YAML OpenAPI spec 生成工具。
- `MCPToolRegistry` 提供最小化 stdio MCP adapter，用于包装 MCP 工具。
- GitHub Actions 会运行 lint、格式检查和测试。
...
│   │   ├── async_loop.py            # AsyncAgentLoop 异步变体
...
│   │   ├── trace.py                 # StepTrace + TraceLogger，Mermaid/HTML 导出
...
│   │   ├── audit.py                 # 工具执行审计日志
│   │   ├── security.py              # 工作区路径解析
│   │   ├── timeout.py               # 工具超时/取消辅助
│   │   ├── sandbox.py               # 受限 Shell 执行模式
│   │   ├── openapi_adapter.py       # OpenAPI spec -> 工具适配器
│   │   └── mcp_adapter.py           # 最小化 stdio MCP 适配器
│   │
│   ├── llm/                         # LLM 抽象层
...
│   │   ├── extractor.py             # 关键词/LLM 记忆抽取器
│   │   ├── manager.py               # 冲突处理与重要度衰减协调器
│   │   ├── composite.py             # 组合记忆后端
...
- **实现了上下文管理与持久化记忆**：采用滑动窗口压缩策略配合规则摘要，使用 SQLite 存储会话、消息、长期记忆和执行追踪，并加入可选向量检索、trace replay/export，通过 660+ 项单元测试覆盖运行时。
